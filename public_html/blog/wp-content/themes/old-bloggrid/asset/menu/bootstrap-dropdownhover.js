...
var Dropdownhover=function(element,options){this.options=options
this.$element=$(element)
var that=this
this.dropdowns=this.$element.hasClass('dropdown-toggle')?this.$element.parent().find('.dropdown-menu').parent('.dropdown'):this.$element.find('.dropdown')
if(!options.onClick){this.dropdowns.each(function(){$(this).on('mouseenter.bs.dropdownhover',function(e){that.show($(this).children('a, button'))})})
this.dropdowns.each(function(){$(this).on('mouseleave.bs.dropdownhover',function(e){that.hide($(this).children('a, button'))})})}else{this.dropdowns.each(function(){$(this).children('a, button').on('click.bs.dropdownhover',function(e){var isActive=$(this).parent().hasClass('open')
isActive?that.hide($(this)):that.show($(this))})})}}
Dropdownhover.TRANSITION_DURATION=300
Dropdownhover.DELAY=150
Dropdownhover.TIMEOUT
Dropdownhover.DEFAULTS={onClick:!1,animations:['fadeInDown','fadeInRight','fadeInUp','fadeInLeft'],}
...
$parent.siblings().each(function(){if(!$(this).hasClass('open')){$(this).find('[data-hover="dropdown"]').attr('aria-expanded',!1)}})
...
$this.attr('aria-expanded',!1)},($this_delay?$this_delay:Dropdownhover.DELAY))}
...
if('ontouchstart' in document.documentElement){Plugin.call($target,$.extend({},$target.data(),{onClick:!0}))}else{Plugin.call($target,$target.data())}})})}(jQuery)